...
def rdc(x, y, f=np.sin, k=20, s=1/6., n=1):
    ...
    # Due to numerical issues, if k is too large,  then rank(fX) < k or rank(fY) < k, so we need
    # to find the largest k such that the eigenvalues (canonical correlations) are real-valued
    k0 = k
    lb = 1
    ub = k
    while True:
