...
xMaxs = [5, 7, 11, 50]
yMaxs = [10, 12, 17, 60]
buckets = [x for x in zip(xMaxs, yMaxs)]
bucketed_data = [[] for _ in range(len(xMaxs))]
for eng, ger in zip(english_processed, german_processed):
    for ix, (xMax, yMax) in enumerate(zip(xMaxs, yMaxs)):
        if (len(eng) <= xMax) and (len(ger) <= yMax):
...
for i in range(generations):
    ...
    ress = translate_model.get_batch(bucketed_data, rand_bucket_ix)
    encoder_inputs, decoder_inputs, target_weights = ress
    ...
    if (i+1) % eval_every == 0:
        for ix, sentence in enumerate(test_data):
            ...
            bucket_id = next(index for index, val in enumerate(xMaxs) if val>=len(sentence))
            # Get RNN model outputs
            encoder_inputs, decoder_inputs, target_weights = test_model.get_batch(
                {bucket_id: [(sentence, [])]}, bucket_id)
            # Get logits
            _, testLoss, output_logits = test_model.step(sess, encoder_inputs, decoder_inputs,
...
plt.plot(loss_generations, trainLoss, 'k-')
