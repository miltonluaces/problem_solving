...
def input_pipeline(bSize, train_logical=True):
    if train_logical:
    ...
    #   min_after_dequeue + (num_threads + a small safety margin) * bSize
...
def cifar_cnn_model(input_images, bSize, train_logical=True):
    ...
    reshaped_output = tf.reshape(norm2, [bSize, -1])
...
# Train step
def step(loss_value, generation_num):
    ...
    step = optimizer.minimize(loss_value)
    return(step)
...
images, targets = input_pipeline(bSize, train_logical=True)
# Get batch test images and targets from pipline
test_images, test_targets = input_pipeline(bSize, train_logical=False)

# Declare Model
print('Creating the CIFAR10 Model.')
with tf.variable_scope('model_definition') as scope:
    # Declare the training network model
    res = cifar_cnn_model(images, bSize)
    ...
    test_output = cifar_cnn_model(test_images, bSize)

# Declare loss function
print('Declare Loss Function.')
loss = cifar_loss(res, targets)
...
train_op = step(loss, generation_num)
...
# Plot loss over time
plt.plot(output_indices, trainLoss, 'k-')
...
plt.plot(eval_indices, testAcc, 'k-')
