...
def SimpleMnist(mnist):
    
    # x : placeholder for the 28x28 image
    x = tf.placeholder(tf.float32, shape=[None, 784])

    # y_ ("y bar") : vector of predicted probability of each digit(0-9) i.e. [0.14, 0.8, 0,0,0,0,0,0,0, 0.06]
    ...
    # Evaluate by comparing the digit with the highest probability in actual (y) and predicted (y_).
    res = tf.equal(tf.argmax(y,1), tf.argmax(y_,1))
    resMean = tf.reduce_mean(tf.cast(res, tf.float32))
    accuracy = sess.run(resMean, feed_dict={x: mnist.test.images, y_: mnist.test.labels})
